...
!###############################################################################
! Added by Ting-Chi Wu 2019/09/03, extended on 2020/03/25 for snow cover/depth
  integer k
  integer tmp_jrec, land_jrec, snowc_jrec, snod_jrec, icec_jrec 
  integer(4) reclen
  integer nx, ny
  real, allocatable :: land(:,:), skint(:,:), snowc(:,:), snod(:,:), icec(:,:)
!###############################################################################
!---------------------------------------------------------------------------
!--- local vars
  character(16) vname
...
!################################################################3  
! Added by Ting-Chi Wu 2019/09/03
...
!       ' vlevtyp=',trim(vlevtyp),' vlev=',vlev,'data=',maxval(tmp),minval(tmp)
    if (trim(vname)=='tmp' .and. trim(vlevtyp)=='sfc') then
       tmp_jrec=jrec
       print *, 'tmp jrec, vlevtyp = ', tmp_jrec, trim(vlevtyp)
    endif   
    if (trim(vname)=='land' .and. trim(vlevtyp)=='sfc') then
       land_jrec=jrec
       print *, 'land jrec, vlevtyp = ', land_jrec, trim(vlevtyp)
    endif   
    if (trim(vname)=='snowc_ave' .and. trim(vlevtyp)=='sfc') then
       snowc_jrec=jrec
       print *, 'snowc jrec, vlevtyp = ', snowc_jrec, trim(vlevtyp)
    endif   
    if (trim(vname)=='snod' .and. trim(vlevtyp)=='sfc') then
       snod_jrec=jrec
       print *, 'snod jrec, vlevtyp = ', snod_jrec, trim(vlevtyp)
    endif   
    if (trim(vname)=='icec' .and. trim(vlevtyp)=='sfc') then
       icec_jrec=jrec
       print *, 'icec jrec, vlevtyp = ', icec_jrec, trim(vlevtyp)
    endif   
  enddo

!  print *,'jrec for sfc tmp=',tmp_jrec
!  print *,'jrec for sfc land=',land_jrec

  allocate(skint(im,jm))
  allocate(land(im,jm))
  allocate(snowc(im,jm))
  allocate(snod(im,jm))
  allocate(icec(im,jm))

  vname='tmp'
  jrec=tmp_jrec
!  print *, 'reading surface tmp jrec =', jrec
  call nemsio_getrechead(gfile,jrec,vname,vlevtyp,vlev,iret)
  call nemsio_readrec(gfile,jrec,tmp,iret=iret)
  skint(:,:)=reshape(tmp,(/im+2*nframe,jm+2*nframe/))

  vname='land'
  jrec=land_jrec
!  print *, 'reading surface land type jrec =', jrec
  call nemsio_getrechead(gfile,jrec,vname,vlevtyp,vlev,iret)
  call nemsio_readrec(gfile,jrec,tmp,iret=iret)
  land(:,:)=reshape(tmp,(/im+2*nframe,jm+2*nframe/))

  vname='snowc_ave'
  jrec=snowc_jrec
!  print *, 'reading surface snow coverage jrec =', jrec
  call nemsio_getrechead(gfile,jrec,vname,vlevtyp,vlev,iret)
  call nemsio_readrec(gfile,jrec,tmp,iret=iret)
  snowc(:,:)=reshape(tmp,(/im+2*nframe,jm+2*nframe/))

  vname='snod'
  jrec=snod_jrec
!  print *, 'reading surface snow depth jrec =', jrec
  call nemsio_getrechead(gfile,jrec,vname,vlevtyp,vlev,iret)
  call nemsio_readrec(gfile,jrec,tmp,iret=iret)
  snod(:,:)=reshape(tmp,(/im+2*nframe,jm+2*nframe/))

  vname='icec'
  jrec=icec_jrec
!  print *, 'reading surface ice coverage jrec =', jrec
  call nemsio_getrechead(gfile,jrec,vname,vlevtyp,vlev,iret)
  call nemsio_readrec(gfile,jrec,tmp,iret=iret)
  icec(:,:)=reshape(tmp,(/im+2*nframe,jm+2*nframe/))

!-- write field to a binary file  

  reclen=4*fieldsize

  print*, 'Writing Skin Temperature (K)'
  open(unit=29,file='CANTMP000000',form='unformatted',access='direct', &
       status='replace',recl=reclen)
  write(29,rec=1) ((skint(i,j),i=1,im+2*nframe),j=jm+2*nframe,1,-1)     
  close(29)

  print*, 'Writing Surface Land'
  open(unit=29,file='LAND000000',form='unformatted',access='direct', &
       status='replace',recl=reclen)
  write(29,rec=1) ((land(i,j),i=1,im+2*nframe),j=jm+2*nframe,1,-1)     
  close(29)

  print*, 'Writing Surface Snow Coverage (%)'
  open(unit=29,file='SNOWC000000',form='unformatted',access='direct', &
       status='replace',recl=reclen)
  write(29,rec=1) ((snowc(i,j),i=1,im+2*nframe),j=jm+2*nframe,1,-1)     
  close(29)

  print*, 'Writing Surface Snow Depth (m)'
  open(unit=29,file='SNOWD000000',form='unformatted',access='direct', &
       status='replace',recl=reclen)
  write(29,rec=1) ((snod(i,j),i=1,im+2*nframe),j=jm+2*nframe,1,-1)     
  close(29)

  print*, 'Writing Surface Ice Coverage (proportional)'
  open(unit=29,file='ICEC000000',form='unformatted',access='direct', &
       status='replace',recl=reclen)
  write(29,rec=1) ((icec(i,j),i=1,im+2*nframe),j=jm+2*nframe,1,-1)     
  close(29)
